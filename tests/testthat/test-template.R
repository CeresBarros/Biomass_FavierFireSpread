...
#    `functionName` is a function's name in your module (e.g., `FavierFireSpreadEvent1`).
# 2. Copy this file to the tests folder (i.e., `m/FavierFireSpread/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("FavierFireSpread")
  path <- list(modulePath = "m",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    FavierFireSpread = list(.saveInitialTime = NA)
  )
  ...
  if (exists("FavierFireSpreadEvent1", envir = .GlobalEnv)) {
    simOutput <- FavierFireSpreadEvent1(mySim)
  } else {
  ...
  if (exists("FavierFireSpreadEvent2", envir = .GlobalEnv)) {
    simOutput <- FavierFireSpreadEvent2(mySim)
  } else {
