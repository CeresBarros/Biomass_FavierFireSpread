...
#    `functionName` is a function's name in your module (e.g., `Biomass_FavierFireSpreadEvent1`).
# 2. Copy this file to the tests folder (i.e., `m/Biomass_FavierFireSpread/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Biomass_FavierFireSpread")
  path <- list(modulePath = "m",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    Biomass_FavierFireSpread = list(.saveInitialTime = NA)
  )
  ...
  if (exists("Biomass_FavierFireSpreadEvent1", envir = .GlobalEnv)) {
    simOutput <- Biomass_FavierFireSpreadEvent1(mySim)
  } else {
  ...
  if (exists("Biomass_FavierFireSpreadEvent2", envir = .GlobalEnv)) {
    simOutput <- Biomass_FavierFireSpreadEvent2(mySim)
  } else {
